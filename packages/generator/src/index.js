...
function normalizeTransport(requested = "stdio") {
  const transport = String(requested ?? "stdio").toLowerCase();

  if (transport === "stdio" || transport === "http") {
    return transport;
  }

  throw new Error(`Unsupported transport: ${requested}. Use "stdio" or "http".`);
}
...
function renderProjectFiles(context) {
  const envFile = renderEnvExample(context.authPreset, context.serverUrl, context.transport);
  const packageJson = renderGeneratedPackageJson(context.projectName, context.transport);
...
function renderGeneratedPackageJson(projectName, transport) {
  const dependencies = {
...
function renderEnvExample(authPreset, serverUrl, transport) {
...
function renderGeneratedReadme(context) {
  ...
  const filterSection = filterSummary
    ? `\n## Generation filters\n\n- ${filterSummary}\n`
    : "";
  const transportLabel = context.transport === "http" ? "MCP Streamable HTTP server on `/mcp`" : "MCP stdio server";
  const transportNotes = context.transport === "http"
    ? "- Start the generated server and connect your MCP client to `http://localhost:$PORT/mcp`.\n"
    : "- Launch the server from a compatible stdio MCP client.\n";
...
${transportNotes}- Extend \`src/client.ts\` if your API needs custom signing or retries.
`;
}
